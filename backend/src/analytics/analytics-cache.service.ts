...
export class AnalyticsCacheService {
  private readonly store = new Map<string, { v: CacheValue; expireAt: number }>()
  ...
  get<T = any>(key: string): T | undefined {
    ...
    if (this.now() >= hit.expireAt) { this.store.delete(key); return undefined }
    return hit.v as T
  }
  ...
  // keys helpers
  keyOverview() { return 'analytics:overview' }
  keyAreas(args: { page: number; pageSize: number; start?: Date; end?: Date; sort?: string; order?: string }) {
