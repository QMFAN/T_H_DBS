...
export class UsersSchemaInitService implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.ensureUsersTable().catch((e) =>
      this.logger.warn(`Create users table skipped: ${(e as Error).message}`),
    );
    await this.ensureColumns().catch((e) =>
      this.logger.warn(`Ensure users columns skipped: ${(e as Error).message}`),
    );
    await this.ensureAdminUser().catch((e) =>
      this.logger.warn(`Ensure admin skipped: ${(e as Error).message}`),
    );
  }
...
CREATE TABLE IF NOT EXISTS users (
  ...
  private async ensureColumns() {
    const required: Array<{ name: string; ddl: string }> = [
      {
        name: 'username',
        ddl: 'ADD COLUMN username VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL',
      },
      {
        name: 'wecom_user_id',
        ddl: 'ADD COLUMN wecom_user_id VARCHAR(100) COLLATE utf8mb4_unicode_ci NULL',
      },
      {
        name: 'password_hash',
        ddl: 'ADD COLUMN password_hash VARCHAR(255) COLLATE utf8mb4_unicode_ci NULL',
      },
      {
        name: 'role',
        ddl: "ADD COLUMN role VARCHAR(20) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'user'",
      },
    ...
    if (existing.has('user_id')) {
      const pk = await this.ds.query(
        `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND CONSTRAINT_NAME = 'PRIMARY'`,
      );
      ...
      this.logger.log(
        'Dropped legacy column users.user_id and ensured PRIMARY KEY(id)',
      );
    ...
    const idxs = await this.ds.query(
  ...
  private async ensureAdminUser() {
    ...
    const iter = 100000;
    const hash = crypto
      .pbkdf2Sync('admin123', salt, iter, 32, 'sha256')
      .toString('hex');
    ...
    this.logger.log(
      'Ensured admin user exists with default password for testing',
    );
  }
}
