...
import type { ImportAnomalyOverviewDto, DuplicateAreaSummaryDto, DuplicateSummaryDto } from './dto/import.dto'
...
export class RedisAnomalyStoreService implements AnomalyStore {
  ...
  register(batchId: string, taskNumericId: number, records: Array<{ anomalyId: string; areaName: string; timestamp: Date; type: ImportAnomalyType; variants: AnomalyVariantRecord[] }>): void {
  ...
  async getOverview(): Promise<ImportAnomalyOverviewDto> {
    ...
    if (!allKeys.length) return { duplicates: { pendingCount: 0, recordCount: 0, anomalyIds: [], areaSummaries: [] }, conflicts: [] }
  ...
  async findById(anomalyId: string): Promise<{ anomalyId: string; taskNumericId: number; batchId: string; areaName: string; timestamp: Date; type: ImportAnomalyType; variants: AnomalyVariantRecord[] } | null> {
  ...
  async resolveOne(anomalyId: string, action: 'skip' | 'overwrite', chooseVariantId?: string): Promise<AnomalyStoreItemResult | null> {
    const raw = await this.redis.get(this.keyAnomaly(anomalyId))
    if (!raw) return null
  ...
  async bulkResolve(type: ImportAnomalyType, action: 'skip' | 'overwrite', anomalyIds: string[]): Promise<Array<AnomalyStoreItemResult & { anomalyId: string }>> {
  ...
  private buildDuplicateSummary(items: Stored[]): DuplicateSummaryDto {
    ...
    const areaSummaries: DuplicateAreaSummaryDto[] = Array.from(areaMap.entries()).map(([areaName, stats]) => ({ areaName, anomalyCount: stats.anomalyCount, recordCount: stats.recordCount }))
    return { pendingCount: items.length, recordCount: totalRecords, anomalyIds: items.map((a) => a.anomalyId), areaSummaries }
  }

  private buildConflictGroups(items: Stored[]): Array<{ areaName: string; anomalies: Array<{ anomalyId: string; timestamp: string; status: 'pending' | 'resolved'; variants: AnomalyVariantRecord[] }> }> {
