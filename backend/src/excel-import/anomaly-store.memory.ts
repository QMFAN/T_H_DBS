...
export class MemoryAnomalyStoreService implements AnomalyStore {
  ...
  async bulkResolve(type: ImportAnomalyType, action: 'skip' | 'overwrite', anomalyIds: string[]): Promise<Array<AnomalyStoreItemResult & { anomalyId: string }>> {
    ...
    for (const id of anomalyIds) {
      ...
      if (r) out.push({ anomalyId: id, batchId: r.batchId, taskNumericId: r.taskNumericId, areaName: r.areaName, timestamp: r.timestamp, resolvedVariant: r.resolvedVariant })
    }
    return out
  }
