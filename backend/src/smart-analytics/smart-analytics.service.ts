...
export class SmartAnalyticsService {
  constructor(
    ...
    private readonly defaultsRepo: Repository<AreaDefaultsEntity>,
  ) {}
  ...
  private async getDbRange(areaId: number): Promise<Range | null> {
    ...
    return {
      start: this.fmt(new Date(row.min)),
      end: this.fmt(new Date(row.max)),
    };
  }
  ...
  private mergeSegments(segs: Range[], toleranceMs: number): Range[] {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (curStart <= lastEnd + toleranceMs) {
        const newEnd =
          this.toDate(cur.end).getTime() > lastEnd ? cur.end : last.end;
        last.end = newEnd;
      } else {
        merged.push(cur);
      }
  ...
  async query(dto: QueryDto) {
    ...
    if (reqEnd < dbStart || reqStart > dbEnd) {
      return {
        success: false,
        message: `请求时间超出数据库范围 (${dbRange.start} 至 ${dbRange.end})`,
        available_range: { min_time: dbRange.start, max_time: dbRange.end },
        data: [],
      };
    }
    const adjusted: Range = {
      start: this.fmt(reqStart < dbStart ? dbStart : reqStart),
      end: this.fmt(reqEnd > dbEnd ? dbEnd : reqEnd),
    };
  ...
  async segments(dto: SegmentsDto) {
    ...
    const toleranceMs =
      ...
      60 *
      1000;
    const segs = this.mergeSegments(
      this.buildSegments(timestamps, toleranceMs),
      toleranceMs,
    );
  ...
  async analyze(dto: AnalyzeDto) {
    ...
    const toleranceMs =
      ...
      60 *
      1000;
    const availableRanges = this.mergeSegments(
      this.buildSegments(timestamps, toleranceMs),
      toleranceMs,
    );
    const missingRanges = this.complementMissing(
    ...
    const threshold = await (async () => {
      ...
      const def = await this.defaultsRepo.findOne({
        where: { area_code: area.code },
      });
      if (!def) return null;
      return {
        tempMin: Number(def.temp_min),
        tempMax: Number(def.temp_max),
        humidityMin: Number(def.humidity_min),
        humidityMax: Number(def.humidity_max),
        source: 'defaults',
      };
    })();
    if (!threshold)
    ...
    const tempAnomalies = data
      .filter(
        (r) =>
          r.temperature != null &&
          (r.temperature < threshold.tempMin ||
            r.temperature > threshold.tempMax),
      )
      .map((r) => ({
        timestamp: r.timestamp,
        temperature: r.temperature,
        type: r.temperature! < threshold.tempMin ? 'low' : 'high',
      }));
    ...
    const humidityAnomalies = data
      .filter(
        (r) =>
          r.humidity != null &&
          (r.humidity < threshold.humidityMin ||
            r.humidity > threshold.humidityMax),
      )
      .map((r) => ({
        timestamp: r.timestamp,
        humidity: r.humidity,
        type: r.humidity! < threshold.humidityMin ? 'low' : 'high',
      }));
    ...
    const buildContinuous = (
      ...
      const getType = (v: number) =>
        v < min ? typeLow : v > max ? typeHigh : 'normal';
      for (let i = 0; i < N; i++) {
        const v = pick(data[i]);
        if (v == null) continue;
        const tp = getType(v);
        if (tp === 'normal') {
          if (startIdx >= 0) {
            if (normalUsed < normalBudget) {
              ...
              const dur =
                (this.toDate(data[endIdx].timestamp).getTime() -
                  this.toDate(data[startIdx].timestamp).getTime()) /
                60000;
              if (dur >= durationMin) {
                res.push({
